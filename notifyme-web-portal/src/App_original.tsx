...
import { Shield, Plus, Settings, LogOut, QrCode, Search, User, Eye, Inbox, Phone, PhoneOff, CreditCard, Bell, Smartphone, Activity, Car, Home, Briefcase, FileText, Lock, Users, Download, HelpCircle, MapPin, TriangleAlert, ShieldAlert, FileSpreadsheet, List, Menu, X } from 'lucide-react';
...
function App() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard'|'tags'|'analytics'|'inbox'|'notifications'|'scan_history'|'vehicle'|'home'|'emergency'|'business'|'subscriptions'|'privacy'|'security'|'family'|'downloads'|'support'|'profile'|'settings'>('dashboard');
  ...
  const handleGoogleLogin = async () => {
    try {
      const res = await axios.post(`${API_BASE}/auth/register`, { 
        email: 'user@gmail.com', name: 'Google User', googleId: `simulated_google_123` 
      });
      ...
      if (loggedInUser) {
        ...
        if (error.response?.data?.user) {
            const dummyToken = error.response.data.accessToken || 'mock-token';
            localStorage.setItem('userToken', dummyToken);
            axios.defaults.headers.common['Authorization'] = `Bearer ${dummyToken}`;

            setUser(error.response.data.user);
            setProfileData(error.response.data.user);
            setIsAuthenticated(true);
            fetchTagsAndMessages(error.response.data.user.id);
        }
    }
  };
    ...
    <div className="dashboard-container">
      ...
      <aside className={`sidebar ${mobileMenuOpen ? 'open' : ''}`}>
        ...
        <nav className="nav-menu" style={{ overflowY: 'auto', maxHeight: 'calc(100vh - 140px)', paddingRight: '8px' }}>
          ...
          <div style={{ marginBottom: '16px' }}>
            <button className={`nav-item ${activeTab === 'downloads' ? 'active' : ''}`} onClick={() => { setActiveTab('downloads'); setMobileMenuOpen(false); }}><Download size={20} /> Downloads</button>
      ...
      <main className="main-content">
        ...
        <div className="content-area">
          ...
          {['notifications', 'family', 'downloads'].includes(activeTab) && (
